...
class KillOtherProcess {
	...
	public static void main(String... args) throws Exception {
		...
		ProcessHandle textEditHandle = ProcessHandle.allProcesses()
				.filter(h -> h.info().commandLine().map(cmd -> cmd.contains("notepad.exe")).orElse(false)).findFirst()
				.orElseThrow(() -> new IllegalArgumentException("No matching handle found"));

		System.out.println(textEditHandle.info());

		textEditHandle.onExit().thenAccept(h -> System.out.println("Notepad was killed by Java!"));
...
class ProcessId {
    public static void main(String... args) {
		...
		long pidOld = Long.parseLong(name.split("@")[0]);
